...
<div class="mb-6 md:mb-8" id="featured-stories-section">
    <h2 class="text-lg md:text-xl font-semibold mb-4 md:mb-6">Featured Stories</h2>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        ...
        <a href="{{ route('article.show', $article->slug) }}" class="flex flex-col md:flex-row gap-0 bg-base-200 rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-all group">
            ...
            <div class="flex-1 md:w-3/5 flex flex-col justify-center p-4 md:p-5 bg-base-200">
                ...
                <h2 class="text-base md:text-lg lg:text-xl font-bold mb-2 text-base-content group-hover:text-primary transition-colors line-clamp-2">
                    {{ $article->title }}
                </h2>
                @if($article->excerpt)
                    <p class="text-xs md:text-sm text-base-content/80 mb-2 line-clamp-2">
                        {{ $article->excerpt }}
                    </p>
                @else
                    <p class="text-xs md:text-sm text-base-content/80 mb-2 line-clamp-2">
